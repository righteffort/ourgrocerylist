...
class SharedListIntegrationTest {
    ...
    private inner class TestUser(val email: String, val listName: String, val appName: String) {
    ...
    @After
    fun tearDown() {
        runCatching { userA.app.delete() }
        runCatching { userB.app.delete() }
        clearEmulatorData()
    }
    ...
    private suspend fun signIn(app: FirebaseApp, email: String): User {
        ...
        val result = try {
            Firebase.auth.createUserWithEmailAndPassword(email, TEST_PASSWORD).await()
        } catch (_: FirebaseAuthUserCollisionException) {
            Firebase.auth.signInWithEmailAndPassword(email, TEST_PASSWORD).await()
        }
        return User(uid = result.user!!.uid, email = email)
    }

    //  private fun makeFirestore(app: FirebaseApp): FirebaseFirestore =
	...
	// TODO: use httpclient for nicer status code check, clearpersistence though i'm not convinced it is necessary
