...
function makeDeps(overrides: Partial<DeleteListDeps> = {}): DeleteListDeps {
  return {
    ...
    // TODO: fix!
    deleteList: async (_listId) => {},  // eslint-disable-line @typescript-eslint/no-empty-function
    ...overrides,
  };
}
...
describe("deleteListCore", () => {
  it("throws unauthenticated when auth is absent", async () => {
    await expect(
      deleteListCore(makeDeps(), undefined, LIST_ID),
    ).rejects.toMatchObject({ code: "unauthenticated" });
  });

  it("throws invalid-argument when listId is missing", async () => {
    await expect(
      deleteListCore(makeDeps(), { uid: OWNER_UID }, undefined),
  ...
  it("throws not-found when list does not exist", async () => {
    ...
    await expect(
      deleteListCore(deps, { uid: OWNER_UID }, LIST_ID),
