...
import { addEditorCore, type AddEditorDeps, type Editor } from "./addEditorCore.js";
...
describe("addEditorCore", () => {
  it("throws unauthenticated when auth is absent", async () => {
    await expect(addEditorCore(makeDeps(), undefined, LIST_ID, EDITOR_EMAIL))
      .rejects.toMatchObject({ code: "unauthenticated" });
  });

  it("throws invalid-argument when listId is missing", async () => {
    await expect(addEditorCore(makeDeps(), { uid: OWNER_UID }, undefined, EDITOR_EMAIL))
      .rejects.toMatchObject({ code: "invalid-argument" });
  });

  it("throws invalid-argument when editorEmail is missing", async () => {
    await expect(addEditorCore(makeDeps(), { uid: OWNER_UID }, LIST_ID, undefined))
      .rejects.toMatchObject({ code: "invalid-argument" });
  });

  it("throws not-found when list does not exist", async () => {
    const deps = makeDeps({ getListOwnerUid: async (_id) => undefined });
    await expect(addEditorCore(deps, { uid: OWNER_UID }, LIST_ID, EDITOR_EMAIL))
  ...
  it("throws not-found when no account exists for the editor email", async () => {
    const deps = makeDeps({ resolveEmailToUid: async (_email) => { throw userNotFoundError(); } });
    await expect(addEditorCore(deps, { uid: OWNER_UID }, LIST_ID, EDITOR_EMAIL))
      .rejects.toMatchObject({ code: "not-found" });
  });
  ...
  it("appends the editor with uid and email on success", async () => {
    ...
    const deps = makeDeps({
      appendEditor: async (listId, editor) => { appended.push({ listId, editor }); },
    });
    await addEditorCore(deps, { uid: OWNER_UID }, LIST_ID, EDITOR_EMAIL);
    expect(appended).toEqual([{ listId: LIST_ID, editor: { uid: EDITOR_UID, email: EDITOR_EMAIL } }]);
  });
